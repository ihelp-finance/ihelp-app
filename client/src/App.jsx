...
document.title = `iHelp (${
  targetNetwork.name.replace("host", "").charAt(0).toUpperCase() +
  targetNetwork.name.replace("host", "").substr(1).toLowerCase()
})`;
...
// WalletLink provider
const walletLinkProvider = walletLink.makeWeb3Provider(localProviderUrl, 43114);
...
function App(props) {
  ...
  }, [mainnetProvider, address, selectedChainId, readContracts, writeContracts]);
            ...
            description={
              <div style={{ color: "white", fontSize: "18px" }}>
                <b style={{ color: "white", fontSize: "20px" }}>
                  {networkSelected && networkSelected.name.toUpperCase()}
                </b>{" "}
                network is not currently supported. Please click here to switch to the{" "}
                <b style={{ color: "white", fontSize: "20px" }}>{networkLocal && networkLocal.name.toUpperCase()}</b>{" "}
                network.
              </div>
            }
            type="error"
